...
import { notifyPomodoroComplete, notifyBreakComplete } from '$lib/notifications';
...
export async function handleTimerCompletion(heroTask, completedTodayCount) {
	...
	if (currentMode === 'pomodoro') {
		...
		notifyPomodoroComplete(heroTask?.title);
...
export function applyMode(mode) {
	...
	console.log(
		`applyMode: mode=${mode}, duration=${safeDuration}, timerSettings=`,
		timerState?.settings
	);
